app.controller("BlogController",function($scope,$rootScope,$location,$http,$log){
	...
	$scope.addComment = function(ob,commentText)
	{
		$http.post('http://localhost:8081/blogmiddleware/addBlogComment/'+commentText,ob,$scope.commentText).then(function(response){
		 ...
		 $scope.getcomments=function(blogId){
			  ...
			  $http.get('http://localhost:8081/blogmiddleware/getComments/'+blogId).then(function(response){
